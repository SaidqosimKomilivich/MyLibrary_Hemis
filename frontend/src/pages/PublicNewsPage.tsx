            ...
            <main className="pt-10 pb-20 px-6 max-w-7xl mx-auto">
                ...
                <div className="flex items-center justify-between gap-4 mb-8">
                    <form onSubmit={handleSearchSubmit} className="relative w-full max-w-md">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 bg-surface border border-border rounded-xl text-sm text-text placeholder:text-text-muted/60 outline-none focus:border-blue-500 transition-colors shadow-xs"
                        />
                    </form>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-7">
                    {isLoading ? (
                        ...
                        newsList.map((item, i) => {
                            const snippet = item.summary || item.content.replace(/<[^>]*>?/gm, "").slice(0, 150).trim()
