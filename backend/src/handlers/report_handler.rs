...
use crate::dto::report::{AdminDashboardResponse, ReportDashboardResponse, ReportExportParams};
...
pub async fn get_admin_dashboard(
    ...
    let year = q
        .year
        .unwrap_or_else(|| now.format("%Y").to_string().parse().unwrap_or(2025));
    let month = q
        .month
    ...
    let (books_by_category, books_by_language) =
        ReportRepository::get_dashboard_book_stats(p).await?;
...
pub async fn get_public_stats(pool: web::Data<PgPool>) -> Result<HttpResponse, AppError> {
    let p = pool.get_ref();
...
pub async fn export_excel(
    ...
    let start_date = match &params.start_date {
        Some(d) => NaiveDate::parse_from_str(d, "%Y-%m-%d").unwrap_or(today),
        None => today,
    };
    ...
    if is_rentals {
        ...
        if data.is_empty() {
            return Err(AppError::NotFound(
                "Tanlangan kunlar oralig'ida ijara ma'lumotlari topilmadi".into(),
            ));
        }
        ...
        for (i, row) in data.iter().enumerate() {
            ...
            let pos_display = if pos_display.is_empty() {
                "-".to_string()
            } else {
                pos_display
            };
            ...
            let _ =
                worksheet.write_string(row_idx, 1, row.user_full_name.as_deref().unwrap_or("-"));
        ...
        if data.is_empty() {
            return Err(AppError::NotFound(
                "Tanlangan kunlar oralig'ida keldi-ketdi ma'lumotlari topilmadi".into(),
            ));
        }
        ...
        let data =
            ReportRepository::get_submissions_by_teacher(p, params.teacher_id.as_deref()).await?;

        if data.is_empty() {
            return Err(AppError::NotFound(
                "Ushbu o'qituvchi bo'yicha taqdim etilgan kitoblar topilmadi".into(),
            ));
        }
        ...
        if data.is_empty() {
            return Err(AppError::NotFound(
                "Filtrlarga mos foydalanuvchilar yo'q".into(),
            ));
        }
        ...
        let _ =
            worksheet.write_string_with_format(0, 3, "Yo'nalish / Mutaxassislik", &header_format);
        ...
        for (i, row) in data.iter().enumerate() {
            ...
            let _ =
                worksheet.write_string(row_idx, 2, row.department_name.as_deref().unwrap_or("-"));
            let _ =
                worksheet.write_string(row_idx, 3, row.specialty_name.as_deref().unwrap_or("-"));
            let _ = worksheet.write_string(row_idx, 4, row.group_name.as_deref().unwrap_or("-"));
            let _ =
                worksheet.write_string(row_idx, 5, row.education_form.as_deref().unwrap_or("-"));
            let _ =
                worksheet.write_string(row_idx, 6, row.staff_position.as_deref().unwrap_or("-"));
        ...
        .await?;
        if data.is_empty() {
            return Err(AppError::NotFound("Kitoblar tarmog'i bo'sh".into()));
        }
        ...
        for (i, row) in data.iter().enumerate() {
            ...
            let _ =
                worksheet.write_string(row_idx, 5, row.shelf_location.as_deref().unwrap_or("-"));
        }
        let _ = worksheet.autofit();
    } else if is_overdue {
        ...
        if data.is_empty() {
            return Err(AppError::NotFound(
                "Muddatidan o'tgan ijaralar topilmadi".into(),
            ));
        }
        ...
        if data.is_empty() {
            return Err(AppError::NotFound("Kitob so'rovlari topilmadi".into()));
        }
        ...
        for (i, row) in data.iter().enumerate() {
            let row_idx = (i + 1) as u32;
            let type_str = if row.request_type == "physical" {
                "Kitob/qog'oz"
            } else {
                "Elektron/Audio"
            };
            let status_str = match row.status.as_str() {
                "pending" => "Kutilmoqda",
                "processing" => "Ko'rilmoqda",
                "ready" => "Tayyor/Qondirildi",
                "rejected" => "Rad etildi",
                _ => &row.status,
            };
            ...
            let _ =
                worksheet.write_string(row_idx, 6, row.employee_comment.as_deref().unwrap_or("-"));
        }
        let _ = worksheet.autofit();
    } else if is_gate {
        ...
        if data.is_empty() {
            return Err(AppError::NotFound(
                "Ushbu davrda kirib-chiqish ma'lumotlari mavjud emas".into(),
            ));
        }
        ...
        let data =
            ReportRepository::get_books_added_by_staff(p, params.staff_id.as_deref()).await?;
        if data.is_empty() {
            return Err(AppError::NotFound(
                "Ushbu xodim tomonidan qo'shilgan kitoblar topilmadi".into(),
            ));
        }
        ...
        if data.is_empty() {
            return Err(AppError::NotFound(
                "Ushbu xodim bo'yicha kitoblar topilmadi".into(),
            ));
        }
        ...
        if data.is_empty() {
            return Err(AppError::NotFound(
                "Xodimlar statistikasi mavjud emas".into(),
            ));
        }

        let _ = worksheet.write_string_with_format(0, 0, "Tr", &header_format);
        let _ =
            worksheet.write_string_with_format(0, 1, "Kutubxona xodimi (F.I.SH)", &header_format);
    ...
    let buf = workbook
        .save_to_buffer()
        .map_err(|e| AppError::InternalError(format!("Excel fayl yaratishda xatolik: {}", e)))?;
